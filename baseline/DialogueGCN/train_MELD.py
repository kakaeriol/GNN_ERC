...
def get_MELD_loaders(path, label="sentiment", batch_size=32, num_workers=0, pin_memory=False):
    trainset = MELDDataset("train",path,label)
    train_speakers = trainset.train_speakers # speaker of training set
    testset = MELDDataset("test",path,label)
    testset.train_speakers = train_speakers # assign test set speaker feature with training speaker
    validset = MELDDataset("valid",path,label)
    validset.train_speakers = train_speakers # assign test set speaker feature with training speaker
...
if __name__ == '__main__':
    ...
    parser.add_argument('--labels', default='sentiment', help='what label to predict, must be one of emotion/sentiment')
    ...
    if args.graph_model:
        seed_everything()
        glv_pretrained = np.load(open('MELD_features/glv_embedding_matrix2', 'rb'),allow_pickle=True)
    ...
    if args.labels != "emotion" and args.labels != "sentiment":
        print('labels should be either emotion or sentiment')
        raise NotImplementedError

    if args.class_weight:
        train_loader, valid_loader, test_loader = get_MELD_loaders('MELD_features/MELD.pkl', labels=args.labels,
                                                                            batch_size=batch_size, num_workers=0)
    else:
        train_loader, valid_loader, test_loader = get_MELD_loaders('MELD_features/MELD.pkl',
                                                                            batch_size=batch_size, num_workers=0)
