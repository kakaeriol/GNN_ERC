...
if __name__ == '__main__':
    ...
    # --- PATH --- 
    train_json = os.path.join(conf["base_DailyDialogue_path"], "train.json")
    test_json = os.path.join(conf["base_DailyDialogue_path"], "test.json")
    valid_json = os.path.join(conf["base_DailyDialogue_path"], "valid.json")
    ...
    #
    idata_path = conf["idata_DailyDialogue_path"]
    if not os.path.exists(idata_path):
        os.mkdir(idata_path)
    ale_path = os.path.join(conf["idata_DailyDialogue_path"], "act_label_encoder.pkl") 
    ald_path = os.path.join(conf["idata_DailyDialogue_path"], "act_label_decoder.pkl") 
    ele_path = os.path.join(conf["idata_DailyDialogue_path"], "emotion_label_encoder.pkl") 
    eld_path = os.path.join(conf["idata_DailyDialogue_path"], "emotion_label_decoder.pkl")
    ...
    pickle.dump(tokenizer, open(os.path.join(conf["base_DailyDialogue_path"], 'tokenizer.pkl'), 'wb'))
    ...
    pickle.dump([convSpeakers, convInputSequence, convInputMaxSequenceLength, convActLabels, convEmotionLabels,
                 train_conv_ids, test_conv_ids, valid_conv_ids], open(conf["data_DailyDialogue_path"], 'wb'))
    ...
    np.ndarray.dump(glv_embedding_matrix, open(os.path.join(conf["base_DailyDialogue_path"], 'glv_embedding_matrix2'), 'wb'))
